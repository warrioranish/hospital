...
function documentReadyInit() {
	...
	//price filter
	if (jQuery().slider) {
		jQuery( ".sliders-range-price" ).slider({
			range: true,
			min: 0,
			max: 100000,
			values: [ 1500, 30000 ],
		...
		jQuery( ".slider_price_min" ).val( jQuery( ".sliders-range-price" ).slider( "values", 0 ) );
		jQuery( ".slider_price_max" ).val( jQuery( ".sliders-range-price" ).slider( "values", 1 ) );
	}
...
function windowLoadInit() {
	...
	if (jQuery().flexslider) {
		...
		$introSlider.each(function(index){
			...
			// .addClass('to_animate')
		}); //intro_section flex sliders

		jQuery(".flexslider").each(function(index){
			var $currentSlider = jQuery(this);
			//exit if intro sliders already activated
	...
	setTimeout(function(){
		...
		if ($header.length) {
			...
			//for boxed layouts - show or hide main menu elements if width has been changed on affix
	...
	jQuery('.isotope_container').each(function(index) {
		...
		var layoutMode = ($container.hasClass('masonry-layouts')) ? 'masonry' : 'fitRows';
		$container.isotope({
